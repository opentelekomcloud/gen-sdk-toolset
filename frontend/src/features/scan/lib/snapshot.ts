...
/**
 * `created_at` of a snapshot is when that result first appeared — not when the
 * repository was last read. An unchanged rescan stores no new snapshot and
 * bumps `last_scanned_at` instead, which is what the service's `scanned_at`
 * reports; the two only agree until the first rescan that changes nothing.
 * Rendered in the viewer's locale and timezone (en-GB: 23/07/2026, 09:15 ·
 * de-DE: 23.07.2026, 09:15) — pass `locale` from useI18n().
 */
