...
| **Startup cleanup** | The API's lifespan hook calls `terminate_orphaned_jobs()`, ending every Job left `queued` or `running` by a previous process with `"interrupted by panel restart"`. Without it an orphan blocks its service forever through `uq_active_scan_job_per_service`. A failure there is logged, never fatal to boot. |
