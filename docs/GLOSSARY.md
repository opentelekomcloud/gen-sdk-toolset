...
| `Identity` | Who made a request: `subject` (the token's `sub`), `name` (the login, from `preferred_username`, `name`, `email`, else `sub`) and the granted roles. `name` is what is written to `job.initiated_by` and `ExcludedService.excluded_by` — the request body still carries `initiated_by` and it is ignored, because a self-reported name is not an attribution. |
