...
from tools.domain.exceptions import NotFoundError, RateLimitError, RepositoryError  #
    ...
    def list_files(self, repo: str, branch: str) -> list[str]:
        try:
            ...
            return [
                ...
                if item["path"].startswith(self.prefix)
    ...
    def fetch_content(self, repo: str, path: str) -> str:
        try:
            resp = self.session.get(
                f"{self.api_url}/repos/{repo}/contents/{path}", timeout=30
            )
            resp.raise_for_status()
